...
1. Record the latest comment identifiers and timestamps on every linked issue immediately before running `gh pr ready <number>`.
2. After the pull request is confirmed non-draft and its linked issues are moved to `agent:needs-review`, wait a full 60 seconds. Then retrieve the linked issues' comments again with sufficient pagination and identify comments added or edited since the snapshot. Do not replace this delay with an earlier final refresh.
